...
class Pool{
    public:
        ...
        void match() {
            for (uint32_t i = 0; i < wt.size();i++) wt[i]++;   // 每次匹配时间加1s
            ...
            while(users.size() > 1) {
                ...
                if(!flag) break;    // 匹配过了外层循环直接break
            }
            if (flag) break;     // 找了一轮，没有可用于匹配的，break；
        }
    }
    private:
        vector<User> users;
        vector<int> wt;  // 记录每个用户等待的时间，时间越久匹配范围越大
...
void consume_task() {
    while(true)
    {
        ...
        if(message_queue.q.empty()) {
            ...
            // pool.match();   // 不去掉就是每次来人都会匹配，时间都加一，不太好
        }
    }
